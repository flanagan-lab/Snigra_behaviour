...
If a bout contained chasing, how frequently did it end with chasing? Never!

```{r chasingPresentEnd}
plot(table(chase_data$Chasing_present, chase_data$Chasing_end_numeric))
```
...
```{r ChasingMod}
ChaseMod<- glmer(Chasing_numeric~Group_size+logDuration+Time_of_Day +(1|Trial), 
                    family="binomial",
                    data=bout_data)
...
For this analysis, let's restrict it to those bouts that had some chasing in them.
...
```{r grpsizeEndMod}
EndMod<- glmer(Chasing_end_numeric~Group_size+logDuration+Time_of_Day +(1|Trial), 
                    family="binomial",
                    data=chase_end_data)
