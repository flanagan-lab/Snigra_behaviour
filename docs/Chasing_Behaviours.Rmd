...
court_data <- do.call(rbind, useful_list)
...
chase_data <- chase_data[,-which(colnames(chase_data)=="courtship_events_wrong")]
```
...
behav_data <- merge(court_data, chase_data[,c("combo","media_file_path", "time_in_video", "Duration" , "total_time_of_courtship",
...
intersex<-unlist(as.list(by(missing,missing$courtship_events,function(bout){
...
missing<-missing[missing$courtship_events %in% names(intersex[intersex==TRUE]),]

active_behavs<-c("Wiggle","Pose")

active<-unlist(as.list(by(missing, missing$courtship_events, function(bout, active_behavs){
...
missing<-missing[missing$courtship_events %in% names(active_bouts),]
table(missing$media_file_path, missing$courtship_events)
```

These three courtship bouts should be in the chase data because they involve both males and females and active behaviours. One of them is missing the Day_filmed info, but that's secondary. 
...
It appears that the missing bout from Trial 4 is not included in the merged dataset because it's not included in the chase data -- that's probably also why it's got NA for the day. The video "D:/Pipefish Mesocosm Film/Trial 4/5-12-20/5-12  AM 01.MTS" was apparently not in the chase results.

What about the other two bouts? Their video is in the chase datasheet.
...
```{r chasingNum}
chase_data$Chasing_numeric<-as.numeric(factor(chase_data$Chasing_present, levels=c("Yes","No")))
chase_data$Chasing_end_numeric<-as.numeric(factor(chase_data$Chasing_end, levels=c("Yes","No")))
```

```{r}
mod1<-glm((chase_data$Chasing_numeric-1)~chase_data$Group_size*chase_data$Time_of_Day, 
          family="binomial")
summary(mod1)
anova(mod1)
```

We can describe the fit using ...
...
newx = seq(min(chase_data$Group_size), max(chase_data$Group_size), length.out=nrow(chase_data))
...
plot(jitter(chase_data$Chasing_numeric-1)~chase_data$Group_size,
     xlab="Group size",
...
polygon(c(rev(newx),newx), c(rev(pl),pu),
     col = "grey90", border = NA)

lines(newx, pm, lwd = 2)
lines(newx, pu, lwd = 2, col = "red")
lines(newx, pl, lwd = 2, col = "red")

# bout ended with chasing
plot(jitter(chase_data$Chasing_end_numeric-1)~chase_data$Group_size,
     xlab="Group size",
