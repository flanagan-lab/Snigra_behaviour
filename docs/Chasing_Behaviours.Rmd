...
```{r variancesPlot, fig.height=7, fig.width=7}
par(mfrow=c(2,2))
...
```{r variancesPlotFlipped, fig.height=7, fig.width=7}
par(mfrow=c(2,2))
...
```{r predictedsByCats, fig.height=7, fig.width=7}
par(mfrow=c(2,2))
...
Based on Table \@ref(tab:EndMod), the lower likelihood is pretty low compared to the estimates and the upper likelihood, suggesting that variance is possibly not well estimated.  
...
Not particularly linear but also looking like there is no effect (which matches the model summary results).

**the variances of data (transformed by the link function) are homogeneous across categories**

This is similar -- we plot the predicted values across each of the categories and ensure similar variances.

```{r variancesEnd, fig.width=7}
par(mfrow=c(1,2))
boxplot(mydata$logit~mydata$Trial)
boxplot(mydata$logit~mydata$Time_of_Day)
```

The variances look mostly ok here -- more variation in trial 3 than the others, but not dramatically so. 


**No major outliers are present**

We can look at the distribution of residuals and see that they are reasonably normal (Fig. \@ref(fig:histResidEnd)), although possibly a bit bimodal. 
