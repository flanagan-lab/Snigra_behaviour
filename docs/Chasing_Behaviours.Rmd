...
```
```{r}
write.csv(behav_data, "combined_behavioural_data.csv", row.names = FALSE)
```
...
```{r plotChasing, fig.height=5, fig.width=7, fig.cap="The probability of chasing occurring as a result of group size and duration. The bottom row plots the probability of a bout ending in chasing (given that it contained some chasing behaviour in the first place)."}
